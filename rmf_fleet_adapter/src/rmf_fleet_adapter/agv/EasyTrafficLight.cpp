  ...
  if (location.has_value())
  {
    ...
    for (const auto& slice : slices)
    {
      try
      {
        ...
        const auto delta_delay = hooks.node->rmf_now() - expected_time;
        new_cumulative_delay = existing_cumulative_delay + delta_delay;
        break;
      }
...
void EasyTrafficLight::Implementation::Shared::publish_fleet_state() const
{
  ...
  hooks.fleet_state_pub->publish(std::move(fleet_state));
}
